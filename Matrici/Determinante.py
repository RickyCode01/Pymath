#modulo per calcolo determinante matrice 20/08/2022
...
def Sottomatrice(matrice, riga, colonna):
    sottomatrice = []
        ...
        if indice <= len(matrice):
            if sviluppo == 1: #sviluppo rispetto alla colonna dell'indice
                for nrighe in range(len(matrice)):
                    determinante += pow(-1,nrighe+indice) * matrice[nrighe][indice] * DeterminanteLaplace(Sottomatrice(matrice, nrighe, indice), sviluppo, indice-1, determinante)
            elif sviluppo == 0:
                for ncoln in range(len(matrice[0])):
                    determinante += pow(-1,ncoln+indice) * matrice[indice][ncoln] * DeterminanteLaplace(Sottomatrice(matrice, indice, ncoln), sviluppo, indice-1, determinante)
            else:
